...
class PaymentAcquirer(models.Model):
    ...
    provider = fields.Selection(selection_add=[('conekta', 'Conekta'), ('conekta_oxxo', 'Conekta oxxo'),('conekta_spei','Conekta SPEI')])
    ...
    def conekta_oxxo_form_generate_values(self, tx_values):
        ...
        conekta_tx_values.update({'phone': tx_values.get('partner_phone') or (tx_values.get('partner') and tx_values.get('partner').mobie),})
    ...
    def conekta_form_generate_values(self, tx_values):
        ...
        temp_conekta_tx_values = {
            ...
            'phone': tx_values.get('partner_phone') or (tx_values.get('partner') and tx_values.get('partner').mobie),
        }
...
class PaymentTransaction(models.Model):
    ...
    def _check_conekta_form_get_invalid_parameters(self, data):
        ...
        if 'phone' in data and len(data['phone']) < 10:
            invalid_parameters.append(('phone', data.get('phone'), self.partner_id.phone or self.partner_id.mobile))    
    ...
    def _conekta_s2s_validate_tree(self, tree):
        ...
        if status == 'paid':
            ...
            expires_at = tree.charges[0].payment_method['expires_at']
            if expires_at:
                vals.update({'conekta_oxxo_expire_date' : datetime.fromtimestamp(int(expires_at)).strftime('%Y-%m-%d')})
