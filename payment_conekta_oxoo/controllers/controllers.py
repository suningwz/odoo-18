...
class PortalConekta(CustomerPortal):
    ...
    def portal_payment_transaction_detail(self, transaction_id, report_type=None, download=False, **kw):
        ...
        if document_sudo.acquirer_id.provider=='conekta_spei':
            return self._show_report(model=document_sudo, report_type=report_type, report_ref='payment_conekta_oxoo.action_report_payment_transaction_spei', download=download)
        else:
...
class Conekta(http.Controller):
    ...
    def conekta_spei_form_feedback(self, **post):
        _logger.info('Beginning form_feedback with post data %s', pprint.pformat(post)) 
        request.env['payment.transaction'].sudo().form_feedback(post, 'conekta_spei')
        return werkzeug.utils.redirect('/payment/process')
