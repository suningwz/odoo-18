...
class TransactionOpenpay(models.Model):
    ...
    def _get_report_base_filename(self):
        self.ensure_one()
        return self.paynet_reference or self.reference
    ...
    def _create_openpay_charge(self, acquirer_ref=None, token_id=None,
        ...
        try:
            if self.acquirer_id.name == 'Openpay':
                # Create the card
            ...
            if self.acquirer_id.name == 'Paynet':
                charge = openpay.Charge.create_as_merchant(
                    customer=customer,
                    method="store",
                    amount=self.amount,
            ...
            msg = "La transacción no pudo ser completada, por favor contacte " \
                  "a su banco"
            # error_message = self.openpay_card_error(msg)
