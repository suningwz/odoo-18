...
PaymentForm.include({
    _createOpenpayToken: function (ev, $checkedRadio, addPmEvent) {
        var self = this;
        if (ev.type === 'submit') {
...
//        console.log("Merchant: " + formData.openpay_merchant_id)
//        console.log("Llave Publica " + formData.openpay_public_key)
        OpenPay.setId(formData.openpay_merchant_id);
        OpenPay.setApiKey(formData.openpay_public_key);
        OpenPay.setSandboxMode(false);

        var deviceSessionId = OpenPay.deviceData.setup();
        formData['device_session_id'] = deviceSessionId

        var cc_expiry = formData.cc_expiry.split(" / ")

        let create_parameters_object = {
            "card_number": formData.cc_number.replace(/ /g, ''),
            "holder_name": formData.holdername,
            "expiration_year": cc_expiry[1],
            "expiration_month": cc_expiry[0],
            "cvv2": formData.cardCVC,
            "address": {
        ...
        var openpaySuccessResponseHandler = function(token) {
            formData['token_id'] = token.data.id
    	...
    	var openpayErrorResponseHandler = function(response) {
        	self.displayError('', response.data.description);
        };

        OpenPay.token.create(create_parameters_object, openpaySuccessResponseHandler, openpayErrorResponseHandler);
    },
    ...
    payEvent: function (ev) {
        ...
        }else if ($checkedRadio.length === 1 && this.isFormPaymentRadio($checkedRadio) && $checkedRadio.data('provider') === 'paynet') {
			var self = this;
			if (ev.type === 'submit') {
	            var button = $(ev.target).find('*[type="submit"]')[0]
	        } else {
    ...
    addPmEvent: function (ev) {
        ...
        if ($checkedRadio.length === 1 && this.isNewPaymentRadio($checkedRadio) && $checkedRadio.data('provider') === 'openpay') {
            this._createOpenpayToken(ev, $checkedRadio, true);
        } else {
            this._super.apply(this, arguments);
        }
    },
