...
class OpenpayController(http.Controller):
    ...
    def openpay_pay_create_charge(self, **post):
        ...
        _logger.info("type :" + str(json_data.get('type', '')))
        if json_data and json_data.get('type', '') == 'charge.succeeded':
            _logger.info("TIENE charge.succeeded")
            tx_obj = request.env['payment.transaction']
            order_id = json_data.get('transaction', {}).get('order_id')
            payment_reference = json_data.get('transaction', {}).get(
                'payment_method', {}).get('reference')
            _logger.info("order_id :" + str(order_id))
            _logger.info("payment_reference :" + str(payment_reference))
            tx = None
            if order_id:
                _logger.info("order_id in if order_id :" + str(order_id))
                tx = tx_obj.sudo().search([
                    ('acquirer_reference', '=', order_id)], limit=1)
            if not tx and payment_reference:
                _logger.info("payment_reference in if not tx and payment_reference:" + str(payment_reference))
                tx = tx_obj.sudo().search([
